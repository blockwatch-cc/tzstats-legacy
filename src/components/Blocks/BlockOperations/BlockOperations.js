...
const BlockOperations = ({ data }) => {
  ...
  function fetchMoreOperations() {
    if (tableData.length != allOperations.length) {
      const sliceIndex = allOperations.length - tableData.length < 10 ? allOperations.length : tableData.length + 10;
        ...
        {tableData.map((item, i) => {
          return (
            <FlexRowSpaceBetween key={i}>
              ...
              <TableCell>{formatCurrency(item.volume)}</TableCell>
              <TableCell>
                <HashLink to={`/operation/${item.hash}`}>{getShortHash(item.hash)}</HashLink>
              </TableCell>
            </FlexRowSpaceBetween>
          );
        })}
