			...
			<Wrapper>
				<Card title={`${opNames[op.type]}`}>
					<FlexRow alignItems={'center'}>
						<TxTypeIcon fontSize={50} mr={40} mt={-15} type={op.type} isSuccess={op.is_success} />
						<Link to={`/${op.data[0].operations.level}`}>
							<DataBox title="Block" valueSize="18px" value={op.data[0].operations.level} />
						</Link>
					</FlexRow>
				</Card>
			</Wrapper>
